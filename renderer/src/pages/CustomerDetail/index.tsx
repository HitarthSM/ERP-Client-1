...
import { ArrowLeft, CreditCard, Receipt, TrendingDown, TrendingUp, ChevronLeft, ChevronRight } from 'lucide-react';
import { toast } from 'sonner';
import { Customers } from '../../api';
import { useAuth } from '../../context/AuthContext';
import { fmt } from '../pos/posHelpers';
import type { CreditStatus, CustomerCreditTransaction } from '../../types';
...
export function CustomerDetailContent({ customerId, onCreditUpdated }: CustomerDetailContentProps) {
  ...
  const isAdminOrManager = ['org_admin', 'org_manager', 'super_admin', 'store_manager'].includes(user?.role ?? '');

  const { data: customer, isLoading, refetch: refetchCustomer } = Customers.useGetById(customerId);
  ...
  async function handlePayment() {
    ...
    if (isNaN(amt) || amt <= 0) { toast.error('Enter a valid amount > 0'); return; }
    if (!payMethod) { toast.error('Select a payment method'); return; }
    await recordTx.mutateAsync({ type: 'payment', amount: amt, paymentMethod: payMethod, note: payNote || undefined });
  ...
  async function handleAdjustment() {
    ...
    if (adjReason === 'Other' && !adjNote.trim()) { toast.error('Enter a note for Other reason'); return; }
    await recordTx.mutateAsync({ type: 'adjustment', amount: amt, note });
    toast.success('Credit adjusted');
    setAdjAmount(''); setAdjNote(''); setShowAdjust(false);
    refetchCustomer();
    onCreditUpdated?.();
  }
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-6 min-h-0">
        ...
        {creditLimit > 0 && (
          <div className={`rounded-xl border p-4 space-y-2 ${creditStatusColor(customer.creditStatus)}`}>
            ...
            <div className="space-y-1 text-xs">
              {[['Limit', fmt(creditLimit)], ['Balance (owed)', fmt(creditBalance)], ['Available', fmt(available)]].map(([k, v]) => (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex gap-1 border-b border-border mb-4">
          {(['bills', 'transactions'] as const).map((tab) => (
            <button
              key={tab}
              type="button"
            ...
            >
              {tab === 'bills' ? 'Bills' : 'Credit Transactions'}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-3">
            {billsLoading ? (
              ...
              <>
                      ...
                      {billsData.items.map((bill: Bill) => (
                        <tr key={bill.id} className="border-t border-border hover:bg-muted/30 transition">
                          <td className="px-3 py-2 text-muted-foreground">
                            {bill.billedAt ? new Date(bill.billedAt).toLocaleDateString() : new Date(bill.createdAt ?? '').toLocaleDateString()}
                          </td>
                ...
                <Pagination page={billsPage} totalPages={billsData.totalPages} onChange={setBillsPage} />
              </>
            )}
          </div>
        )}
              ...
              <>
                <div className="rounded-xl border border-border overflow-hidden">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {txData.items.map((tx: CustomerCreditTransaction) => {
                        const isDebit = tx.type === 'credit_sale' || (tx.type === 'adjustment' && tx.amount > 0);
                        return (
                          <tr key={tx.id} className="border-t border-border hover:bg-muted/30 transition">
                            ...
                            <td className={`px-3 py-2 tabular-nums font-semibold ${tx.type === 'payment' ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <Pagination page={txPage} totalPages={txData.totalPages} onChange={setTxPage} />
              </>
            )}
          </div>
        )}
